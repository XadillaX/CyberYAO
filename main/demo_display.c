...
static void refresh(void) {
  ...
  lv_obj_set_style_text_color(
      s_info, dark ? lv_color_white() : lv_color_black(), 0);
  lv_label_set_text_fmt(
      s_info,
      "%s\n\nBACKLIGHT %d%%\n\nOK: NEXT COLOR\nUP/DOWN: LIGHT",
      COLOR_NAME[s_color_idx],
      BL_LEVELS[s_bl_idx]);
}
...
void demo_display_enter(void) {
  ...
  s_swatch =
      ui_pixel_panel_create(s_scr, 18, 58, 204, 188, COLORS[s_color_idx]);

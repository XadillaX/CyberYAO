...
static esp_err_t teardown_step(esp_err_t first_error, esp_err_t error,
    ...
    for (;;) {
        ...
        if (xQueueReceive(s_audio_events, &event, wait) == pdTRUE) {
            ...
            if (event == AUDIO_EVENT_PLAY_COINS && s_audio_ready) {
                ...
                ESP_LOGI(TAG, "铜钱音效结束 t=%lu，历时=%lu ms",
                         (unsigned long)now_ms(),
                         (unsigned long)(now_ms() - audio_started_ms));
                ambient_offset = 0;
            }
            continue;
        }
        ...
        if (count > 1024U) count = 1024U;
        if (bsp_audio_write(
                yaogui_ambient_sound_pcm + ambient_offset,
                count * sizeof(int16_t)) != ESP_OK) {
...
static void process_key(const key_event_t *key)
{
    ...
    if (s_model.reading.open && key->event == BSP_BTN_CLICK) {
        if (key->button == BSP_BTN_UP &&
            yaogui_view_reading_scroll(s_view, -1)) {
            return;
        }
        if (key->button == BSP_BTN_DOWN &&
            yaogui_view_reading_scroll(s_view, 1)) {
            return;
        }
    }
    ...
    if (key->event == BSP_BTN_CLICK && key->button == BSP_BTN_UP) {
        ...
        request_random = yaogui_model_key(
            &s_model, YAOGUI_KEY_OK_LONG,
            s_last_activity_ms, s_last_activity_ms);
    }
    if (request_random) {
        const bool request = true;
        ESP_LOGI(TAG, "起卦开始 t=%lu", (unsigned long)s_last_activity_ms);
        if (xQueueSend(s_requests, &request, 0) != pdTRUE) {
            yaogui_model_fail(&s_model, YAOGUI_ERROR_RANDOM_SOURCE,
                              s_last_activity_ms);
        } else {
...
{
    ESP_LOGI(TAG, "随机返回 t=%lu，历时=%lu ms",
             (unsigned long)now_ms(),
             (unsigned long)(now_ms() - s_model.started_ms));
    if (result->succeeded) {
        yaogui_model_complete(&s_model, result->values,
                              YAOGUI_SOURCE_ESP32_RF, now_ms());
    } else {
...
static void tick(lv_timer_t *timer)
{
    ...
    if (phase_before_tick == YAOGUI_ROLLING &&
        s_model.phase == YAOGUI_RESULT) {
        ESP_LOGI(TAG, "动画落定 t=%lu，历时=%lu ms",
                 (unsigned long)now_ms(),
                 (unsigned long)(now_ms() - s_model.started_ms));
    }
...
esp_err_t yaogui_app_start(void)
{
    ...
    if (xTaskCreate(random_task, "yaogui_rng", 4096, NULL, 4,
                    &s_random_task) != pdPASS) {
        s_random_task = NULL;
        release_start_resources();
        return ESP_ERR_NO_MEM;
    }
    if (xTaskCreate(audio_task, "yaogui_audio", 6144, NULL, 5,
                    &s_audio_task) != pdPASS) {
        s_audio_task = NULL;
        release_start_resources();
        return ESP_ERR_NO_MEM;
    ...
    s_audio_ready = bsp_audio_init() == ESP_OK &&
                    bsp_audio_set_format(
                        YAOGUI_COIN_SOUND_RATE, 16, 1) == ESP_OK;
    if (s_audio_ready) {

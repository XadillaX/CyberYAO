...
static void sleep_task(void* arg) {
  ...
  for (;;) {
    ...
    if (command == SLEEP_COMMAND_DEEP) {
      ...
      char text[96];
      snprintf(
          text, sizeof(text), "Deep sleep failed:\n%s", esp_err_to_name(err));
      set_status(text);
...
void demo_low_power_enter(void) {
  ...
  for (int i = 0; i < 2; i++) {
    s_mode_cards[i] =
        ui_pixel_panel_create(panel, 7, 56 + i * 54, 176, 42, UI_PAPER);
  ...
  s_busy = false;
  if (!s_task &&
      xTaskCreate(sleep_task, "demo_sleep", 3072, NULL, 4, &s_task) != pdPASS) {
...
void demo_low_power_key(bsp_btn_t btn, bsp_btn_ev_t ev) {
  ...
  if (btn == BSP_BTN_UP || btn == BSP_BTN_DOWN) {
    ...
    uint32_t command =
        s_selected == 0 ? SLEEP_COMMAND_LIGHT : SLEEP_COMMAND_DEEP;

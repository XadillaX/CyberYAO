...
// 取底层面板句柄。想直接 esp_lcd_panel_draw_bitmap 画,或接 LVGL 以外的 GUI
// 时用。 未初始化返回 NULL。
...
// ---------------------------------------------------------------------------
// 前向声明:LVGL 里 lv_display_t 是 `typedef struct _lv_display_t
// lv_display_t;`, 故此处用 struct 形式即可,避免本头文件强行 include lvgl.h。

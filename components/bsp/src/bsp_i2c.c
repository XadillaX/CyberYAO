...
esp_err_t bsp_i2c_init(void) {
  ...
  if (e != ESP_OK) {
    ESP_LOGE(TAG,
             "I2C 总线创建失败 (%s) —— 检查 SDA=GPIO%d / SCL=GPIO%d "
             "是否被别的外设占用",
             esp_err_to_name(e),
             BSP_I2C_SDA,
             BSP_I2C_SCL);
...
esp_err_t bsp_i2c_scan(void) {
  ...
  //   紧接着 err 清理路径 i2c_master_bus_destroy() 拿着它调
  //   i2c_common_deinit_pins(), 把 SDA/SCL 从 I2C
  //   外设上解绑。此后每次事务都卡在 "clear bus failed", ES8311 与 CW2017
  //   一起失联。 (IDF v5.5.3 实测;症状是满屏 reset hardware
  //   failed,极难反查到扫描这一步。)
  //
  // 正式总线上直接扫是安全的:i2c_master_probe() 收尾会把 status 复位成
  // I2C_STATUS_DONE("in order not influence next time transaction"),NACK
  // 不留残留; 它临时设的 100kHz 时序也会被下一次设备事务按各自的 scl_speed_hz
  // 覆盖。
  if (!s_bus) {

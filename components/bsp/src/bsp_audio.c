...
static esp_err_t i2s_full_duplex_init(void) {
  ...
  if ((e = i2s_channel_init_std_mode(s_rx, &std)) != ESP_OK) {
    ESP_LOGE(TAG, "i2s rx 初始化失败: %s", esp_err_to_name(e));
    return e;
  }
  // esp_codec_dev_open 内部重配前会先 i2s_channel_disable,而 disable
  // 要求通道处于 RUNNING;刚 init 的通道是 READY,会打一条 "channel has not been
  // enabled yet" 错误日志。 这里先 enable 一次让那次 disable 合法(此时 codec
  // 未配,不出声)。
...
esp_err_t bsp_audio_init(void) {
  ...
  const audio_codec_ctrl_if_t* ctrl =
      audio_codec_new_i2c_ctrl(&(audio_codec_i2c_cfg_t){
          .port = BSP_I2C_PORT,
  ...
  if (!ctrl) {
    ESP_LOGE(TAG,
             "ES8311 控制口创建失败 —— 用 bsp_i2c_scan() 确认 0x%02X 是否应答;"
  ...
  const audio_codec_data_if_t* data =
      audio_codec_new_i2s_data(&(audio_codec_i2s_cfg_t){
          .port = BSP_I2S_PORT,
          .tx_handle = s_tx,
          .rx_handle = s_rx,
      });
...
esp_err_t bsp_audio_set_format(uint32_t hz, uint8_t bits, uint8_t ch) {
  ...
  //   驱动已按采样率与 MCLK 精确算好,覆写会导致 ADC/DAC
  //   时序错乱、录音回放全是杂音。 这里只设麦克风模拟 PGA 增益。
  ...
  s_bits = bits;
  s_ch = ch;
  ESP_LOGI(TAG, "codec 打开 %" PRIu32 "Hz/%ubit/%uch", hz, bits, ch);
  return ESP_OK;
}

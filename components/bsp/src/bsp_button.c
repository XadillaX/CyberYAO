...
// oneshot 句柄。谁第二个调 adc_oneshot_new_unit() 谁就拿到 "adc1 is already in
// use"。
static adc_oneshot_unit_handle_t s_adc;
static adc_cali_handle_t s_cali;

// 电压读取的衰减档必须与 button 组件内部的 ADC_BUTTON_ATTEN 一致 ——
// 通道只被配置一次 (由组件在 iot_button_new_adc_device()
// 里下发),两边对不上会让读数与按键阈值错位。
// managed_components/espressif__button/button_adc.c:26 在 C3 上取
// ADC_ATTEN_DB_6+1。
#define BSP_BTN_ATTEN ADC_ATTEN_DB_12  // 量程约 0~3100mV,覆盖松开态

// 每个按键把"哪个键"随回调带回来。button 组件的回调签名固定,故用 usr_data
// 传索引。
...
esp_err_t bsp_button_init(bsp_btn_cb_t cb, void* user) {
  ...
  // 先由 BSP 建 unit,再把句柄交给 button 组件(button_adc.h:adc_handle 非 NULL
  // 即复用), 这样本文件的 bsp_button_read_mv() 也能读同一路 ADC。
  ...
  for (int i = 0; i < BSP_BTN_COUNT; i++) {
    ...
    if (e != ESP_OK || !s_btn[i]) {
      ESP_LOGE(TAG,
               "按键 %d 创建失败 (%s) —— 检查 GPIO%d 的 ADC 配置与分压电阻",
               i,
               esp_err_to_name(e),
               BSP_BTN_ADC_CHANNEL);
    ...
    iot_button_register_cb(
        s_btn[i], BUTTON_LONG_PRESS_START, NULL, cb_long, idx);
  }

...
static const uint8_t s_battery_profile[CW_PROFILE_SIZE] = {
    0x64, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xAD, 0xC7, 0xC8, 0xCA,
    0xBD, 0xB1, 0xC1, 0x94, 0x88, 0xD1, 0xBD, 0x97, 0x88, 0x66, 0x56, 0x4A,
    0x3F, 0x33, 0x26, 0x5C, 0x37, 0xD1, 0x27, 0xD8, 0xCC, 0xB7, 0xCF, 0xB3,
    0xB2, 0xAE, 0xA6, 0x9E, 0x99, 0x97, 0x9B, 0x86, 0x47, 0x1E, 0x17, 0x26,
    0x49, 0x96, 0xD9, 0xE1, 0xDD, 0xDC, 0xD4, 0x59, 0x00, 0x00, 0x90, 0x02,
...
static int cw_read(uint8_t reg, uint8_t* buf, size_t n) {
  if (!s_dev) return -1;
  return i2c_master_transmit_receive(s_dev, &reg, 1, buf, n, 100) == ESP_OK
             ? 0
             : -1;
}
...
static int cw_update_profile(void) {
  ...
  for (size_t i = 0; i < CW_PROFILE_SIZE; i++) {
    ...
    if (val != s_battery_profile[i]) {
      ESP_LOGE(TAG,
               "profile 校验失败:index=%u expected=0x%02X actual=0x%02X",
               (unsigned)i,
               s_battery_profile[i],
               val);
      return -1;
...
esp_err_t bsp_battery_init(void) {
  ...
  if (cw_read(CW_REG_VERSION, &ver, 1) != 0) {
    ESP_LOGW(TAG,
             "CW2017 未应答 —— 用 bsp_i2c_scan() 确认 0x%02X 是否在线;"
             "无电量计的板子可忽略本项",
             BSP_I2C_CW2017_ADDR);

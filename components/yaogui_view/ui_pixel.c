...
static lv_obj_t* block(
    lv_obj_t* parent, int x, int y, int w, int h, uint32_t color) {
...
lv_obj_t* ui_pixel_label(lv_obj_t* parent,
                         const char* text,
...
lv_obj_t* ui_pixel_screen_create(const char* title) {
  ...
  lv_obj_set_style_bg_color(scr, lv_color_hex(0xE8D7B4), 0);
  lv_obj_set_style_border_width(scr, 0, 0);
  lv_obj_set_style_pad_all(scr, 0, 0);
  return scr;
}

lv_obj_t* ui_pixel_panel_create(
    lv_obj_t* parent, int x, int y, int w, int h, uint32_t color) {
...
void ui_pixel_set_selected(lv_obj_t* panel, bool selected, bool enabled) {
  ...
  lv_obj_set_style_border_color(
      panel, lv_color_hex(selected ? 0xFFFFFF : UI_INK), 0);
}

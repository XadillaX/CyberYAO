...
static uint32_t random_range(uint32_t* state,
                             uint32_t minimum,
                             uint32_t maximum) {
...
void yaogui_model_complete(yaogui_model_t* model,
                           ...
                           yaogui_random_source_t source,
                           uint32_t now_ms) {
  if (model->phase != YAOGUI_ROLLING || model->request_finished ||
      source == YAOGUI_SOURCE_NONE)
    return;
...
void yaogui_model_fail(yaogui_model_t* model,
...
void yaogui_model_tick(yaogui_model_t* model, uint32_t now_ms) {
  ...
  if (landing_elapsed < 0 ||
      (uint32_t)landing_elapsed < YAOGUI_LANDING_TOTAL_MS)
    return;
...
uint8_t yaogui_model_frame(const yaogui_model_t* model,
                           uint32_t now_ms,
                           size_t shell_index) {
...
static void rolling_motion(const yaogui_model_t* model,
  ...
  uint32_t flip = (elapsed * (uint32_t)(7U + index * 2U) / 10U +
                   (uint32_t)model->angle_start[index]) %
                  512U;
  uint32_t folded = flip <= 256U ? flip : 512U - flip;
  uint16_t perspective = (uint16_t)(220U + height * 36U / model->height[index]);
  uint16_t compression =
      (uint16_t)(32U + (folded > 128U ? folded - 128U : 128U - folded) *
                           (perspective - 32U) / 128U);
  motion->scale_x = compression;
  motion->scale_y = perspective;
  motion->belly = flip >= 128U && flip < 384U;
  motion->shadow_scale =
      (uint16_t)(220U - height * 105U / model->height[index]);
  motion->shadow_opa = (uint8_t)(95U - height * 60U / model->height[index]);
}

static void separate_collision(const yaogui_model_t* model,
                               uint32_t elapsed,
                               size_t shell_index,
                               yaogui_shell_motion_t* motion) {
...
void yaogui_model_motion(const yaogui_model_t* model,
                         uint32_t now_ms,
                         size_t shell_index,
                         yaogui_shell_motion_t* motion) {
  if (!motion) return;
  *motion = (yaogui_shell_motion_t){
      ...
      .belly = model && shell_index < YAOGUI_SHELL_COUNT
                   ? yaogui_model_frame(model, now_ms, shell_index) != 0
                   : false,
  };
  ...
  rolling_motion(model, landing_start - model->started_ms, shell_index, &start);
  uint32_t progress = (uint32_t)local * 256U / YAOGUI_LANDING_MS;
  uint32_t eased = 256U - ((256U - progress) * (256U - progress) / 256U);
  motion->x = (int16_t)(start.x + ((model->target_x[shell_index] - start.x) *
                                   (int32_t)eased) /
                                      256);
  int32_t base_y =
      start.y +
...
static const legacy_hexagram_t HEXAGRAMS[64] = {
    {2,
     "坤",
     "元亨。利牝马之贞。君子有攸往，先迷后得主，利。西南得朋，东北丧朋。安贞，"
     "吉。"},
...
bool yaogui_hexagram_from_lines(const yaogui_line_t lines[YAOGUI_LINE_COUNT],
                                bool changed,
...
static bool append_reading_text(yaogui_reading_page_t* page,
  ...
  int written = snprintf(page->text + used,
                         sizeof(page->text) - used,
                         "%s%s%s%s%s",
                         used ? "\n\n" : "",
                         primary ? "主读·" : "",
                         heading ? heading : "",
...
static bool add_full_hexagram_page(yaogui_reading_plan_t* plan,
                                   yaogui_reading_page_type_t type,
                                   const yaogui_hexagram_t* hexagram,
                                   bool guaci_primary,
                                   uint8_t primary_line_mask,
                                   bool special_primary) {
  ...
  if (hexagram->special &&
      !append_reading_text(page, NULL, hexagram->special, special_primary)) {
    return false;
  }
  plan->count++;
  return true;
...
bool yaogui_reading_build(const yaogui_line_t lines[YAOGUI_LINE_COUNT],
  ...
  if (!add_full_hexagram_page(plan,
  ...
  if (moving_count > 0) {
    ...
    if (!add_full_hexagram_page(plan,
                                ...
                                changed,
                                moving_count == 3 || (moving_count == 6 &&
                                                      primary->special == NULL),
                                changed_line_mask,
                                false)) {
      return false;
    }
...
bool yaogui_model_key(yaogui_model_t* model,
                      yaogui_key_t key,
                      uint32_t now_ms,
                      uint32_t seed) {
  ...
  return key == YAOGUI_KEY_OK && yaogui_model_start_seeded(model, now_ms, seed);
}

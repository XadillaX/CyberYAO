...
- Added a complete standby experience with a daytime sundial, nighttime clepsydra, live time and battery data, and build-time lunar calendar, sexagenary-cycle, solar-term, and daily activity data from `lunar-javascript`. Added Bluetooth time synchronization with a global animated status indicator, distinct manual and idle-timeout brightness behavior, a second-stage screen-off timeout, and corrected wake, standby-return, and hold-to-reset button states. Every UI string now passes through the font coverage gate to prevent missing glyphs and worst-case text overflow on the device.

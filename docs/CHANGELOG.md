...
- Removed hardware demo pages that were outside every build graph and isolated pixel-math code already discarded by the linker; firmware and simulator now share one generated-source manifest. The standby view caches unchanged date, time, calendar, battery, and day/night layout state instead of resetting static LVGL objects every 50 ms, while wall-clock conversion reuses its result within the same second. The main UI font no longer duplicates glyphs used only by the standby calendar. Existing visuals, controls, and timing remain unchanged.
- The calendar now shows a solar-term name on its transition date and otherwise counts down to the next term (for example, “eleven days before Autumn Equinox”), with host coverage, a full-battery simulator capture, and matching font-glyph gates.

## v1.1.0 - 2026-09-12

- Replaced the always-on BLE Current Time Service with phone-assisted Wi-Fi time sync. An invalid system clock now opens the `CyberYAO-Time` access point automatically at startup, while holding DOWN still opens it manually for two minutes. Removed the join QR code and LVGL QR configuration in favor of an overflow-safe, three-step pixel-text panel that retains `192.168.4.1` as the fallback address. The initial and between-cast screens now show contextual controls for OK to cast, UP for standby, hold OK to reset, hold DOWN to sync time, and hold UP for Recovery. DHCP advertises the device as DNS, a lightweight wildcard DNS server resolves names to the portal, and HTTP routes handle iOS `hotspot-detect`, Android `generate_204`/`gen_204`, and Windows `connecttest` probes. The dependency-free embedded page submits the phone's local time automatically, provides animated status and manual retry, and honors `prefers-reduced-motion`; DNS, HTTP, and the access point stop after success or timeout. Software restarts preserve system time, but the device has no backup RTC, so a complete power loss requires time sync again.
- Added a complete standby experience with a daytime sundial, nighttime clepsydra, live time and battery data, and build-time lunar calendar, sexagenary-cycle, solar-term, and daily activity data from `lunar-javascript`. Added a global animated time-sync status indicator, distinct manual and idle-timeout brightness behavior, a second-stage screen-off timeout, and corrected wake, standby-return, and hold-to-reset button states. Every UI string now passes through the font coverage gate to prevent missing glyphs and worst-case text overflow on the device.
- Replaced the three fixed coin landing anchors with seeded two-dimensional motion: each cast now assigns independent horizontal and vertical velocities, reflects from the table boundary, resolves coin-to-coin overlap, and derives the final positions and rotations from that motion. During landing, every coin first turns edge-on, switches to the real result face while compressed, and then unfolds to its full circular aspect ratio instead of changing faces on the final frame.

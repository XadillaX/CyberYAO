...
- Replaced the always-on BLE Current Time Service with on-demand phone Wi-Fi QR time sync. Holding DOWN now opens the `CyberYAO-Time` network for two minutes, while the device shows an open-Wi-Fi join QR code and the fallback address `192.168.4.1`. DHCP advertises the device as DNS, a lightweight wildcard DNS server resolves names to the portal, and HTTP routes handle iOS `hotspot-detect`, Android `generate_204`/`gen_204`, and Windows `connecttest` probes. The dependency-free embedded page uses the parchment, cinnabar, ink, and hard-edged pixel visual language, submits the phone's local time automatically, provides animated status and manual retry, and honors `prefers-reduced-motion`. DNS, HTTP, and the access point stop after success or timeout. RF-backed randomness and the existing standby, wake, and screen-off semantics remain intact.

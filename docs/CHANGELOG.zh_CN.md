...
- 将常驻 BLE Current Time Service 替换为按需手机 Wi-Fi 扫码校时：长按下键后开启两分钟开放热点 `CyberYAO-Time`，屏幕显示开放 Wi-Fi 入网二维码及 `192.168.4.1` 兜底地址。DHCP 广播本机 DNS，轻量 wildcard DNS 将域名解析到校时页，并兼容 iOS `hotspot-detect`、Android `generate_204`／`gen_204` 与 Windows `connecttest` 探测。内嵌无依赖页面采用古纸、朱砂、墨色和像素硬边视觉，自动提交手机本地时间，提供状态动效、失败重试及 `prefers-reduced-motion` 降级；校时成功或超时后关闭 DNS、HTTP 服务与热点。保留射频真随机、待机唤醒和二级熄屏语义。

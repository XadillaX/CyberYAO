...
- 配网页与手机解卦页现在会在构建时自动压缩 HTML、内联 CSS 和 JavaScript，并以可复现的 gzip 数据嵌入固件。浏览器负责解压，页面功能与动态卦象接口保持不变，同时减少固件占用与局域网传输量。

## v1.2.1 - 2026-09-17

- 问卦页不再使用 `localStorage` 保存问卦内容；每次重新打开页面时输入框均为空，避免上一卦的问题出现在下一卦中。WiFi 配网页的旧契功能不受影响。

## v1.2.0 - 2026-09-16

- 将手机校时页替换为赛博六爻木纹配网页：热点名使用设备 MAC 生成稳定后缀（如 `CyberYAO-A1B2C3`），网关与 DNS 使用 `66.66.66.66`，`cyberyao` 域名进入 captive portal；页面支持扫描 WiFi，并在同源 `localStorage` 中按 SSID 保存、自动填入和修改历史密码。设备只在取得 DHCP 地址后持久化凭据，随后通过 NTP 校时；连续 3 次 WiFi 失败或互联网持续 5 分钟不可达时，自动重开配网热点和设备联网提示。
- 新增手机解卦：在详细卦象页双击确认键显示局域网二维码，手机扫码后可补充求测人、问卦内容、解读侧重与深度，并生成可复制到 TraeWork 的结构化六爻 Prompt；高级模式可要求安装六爻 Skill，最终产出适合手机阅读的 HTML 解卦报告。待机页新增 WiFi 状态图标。

## v1.1.1 - 2026-09-12

- 清理未参与构建的硬件演示页和已被链接器回收的孤立像素数学代码；固件与模拟器共用同一份生成源清单。待机页缓存未变化的日期、时间、黄历、电量与昼夜布局，避免 50 ms 动画周期内重复设置静态 LVGL 对象；系统时间转换在同一秒内复用结果。主界面字体不再重复打包仅由待机黄历使用的字符，所有调整保持现有界面和交互逻辑不变。
- 日历改为在交节当天显示节气名，其余日期显示下一节气倒数（如「秋分前十一日」），并增加主机测试、满电模拟器截图和对应字体字形门禁。

## v1.1.0 - 2026-09-12

- 将常驻 BLE Current Time Service 替换为手机 Wi-Fi 校时：系统时间无效时启动即自动开启两分钟开放热点，也可长按下键手动开启 `CyberYAO-Time`。设备端删除入网二维码及 LVGL QR 配置，改为不会溢出的像素风三步文字，并保留 `192.168.4.1` 兜底地址；摇卦初始与等待页补齐 OK 起卦、上键待机、长按 OK 重置、长按下键校时和长按上键恢复提示。DHCP 广播本机 DNS，轻量 wildcard DNS 将域名解析到校时页，并兼容 iOS `hotspot-detect`、Android `generate_204`／`gen_204` 与 Windows `connecttest` 探测。内嵌无依赖页面自动提交手机本地时间，提供状态动效、失败重试及 `prefers-reduced-motion` 降级；校时成功或超时后关闭 DNS、HTTP 服务与热点。软件重启会保留系统时间；设备没有后备 RTC，彻底断电后需重新校时。
- 新增完整待机体验：白昼日晷、夜间刻漏、真实时间与电量，以及由 `lunar-javascript` 构建期生成的农历、干支、节气和宜忌数据；加入全局校时状态提示、手动与超时待机亮度区分、待机二级熄屏，并完善唤醒、返回待机和长按重置的按键状态机。所有界面文字统一进入字体字符集门禁，避免真机缺字和最坏文案溢出。
- 将三枚铜钱的固定落点改为带种子的二维运动：每次投掷分别生成横向与纵向速度，铜钱在桌面边界反弹并处理相互碰撞，最终位置与角度由当次运动自然形成。落地阶段会先转至侧立，在压缩状态切换为真实结果面，再展开为完整圆形，不再于最终帧瞬间翻面。
...
- 完成 CyberYAO 独立应用界面：加入古朴启坛初始化页、八卦桌与三枚铜钱伪三维动画、持续环境音和同步铜钱音效；详细解读改为本卦与之卦各一页并支持按键滚动，完整展示卦辞、六爻及朱熹规则主读标记。
- 将三钱法扩展为从初爻到上爻的六次完整起卦流程：每次落定保存 6／7／8／9 一爻，确认键继续下一爻，第六爻后按准确的上下卦映射显示 64 卦本卦、原文卦辞及老阴老阳形成的变卦。共享 LVGL 底部面板持续显示六爻进度、动爻标记、本卦与变卦，不显示随机源；主机测试覆盖六次状态流转、四类爻、全部 64 卦序号与卦名映射。
- 将「龟壳」真机随机源改为每次投掷短暂启动 ESP32-C3 Wi-Fi STA 射频、等待熵稳定后调用 `esp_fill_random` 取得 3 位结果，再停止并反初始化；不创建网络接口、不扫描、不连接。删除 HTTP、远程随机源、凭据、NVS 与 SoftAP 配网流程，启动直接进入待机，长按 `DOWN` 不再配网，射频生命周期任一步骤失败均直接报错且不回退。模拟器保留异步 Mock 成功／失败场景。

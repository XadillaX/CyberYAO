...
- 新增完整待机体验：白昼日晷、夜间刻漏、真实时间与电量，以及由 `lunar-javascript` 构建期生成的农历、干支、节气和宜忌数据；加入蓝牙校时、全局旋转状态提示、手动与超时待机亮度区分、待机二级熄屏，并完善唤醒、返回待机和长按重置的按键状态机。所有界面文字统一进入字体字符集门禁，避免真机缺字和最坏文案溢出。

...
- 支持手机 Wi-Fi 校时：系统时间无效时自动开启两分钟开放热点，也可长按下键手动
  开启；屏幕分三步提示打开手机 Wi-Fi、连接 `CyberYAO-Time`、等待页面弹出后自动
  校时。DHCP 广播本机 DNS，wildcard DNS 与 iOS、Android、Windows 的联网探测入口
  会自动唤起本地校时页。页面自动提交手机本地年月日时分秒，成功后提示并关闭热点。

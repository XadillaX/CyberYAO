...
- 校时服务仅在长按下键后开启两分钟：设备创建开放热点 `CyberYAO-Time`
  和本地 HTTP 页面，不接入互联网。
...
- 支持手机 Wi-Fi 扫码校时：屏幕二维码可直接加入开放 Wi-Fi；DHCP 广播本机
  DNS，wildcard DNS 与 iOS、Android、Windows 的联网探测入口会自动唤起本地
  校时页。页面自动提交手机本地年月日时分秒，成功后提示并关闭热点。
...
| 下长按 | 开启两分钟 Wi-Fi 扫码校时 |
| 上长按 | 重启并进入 Recovery |

屏幕完全熄灭时，任意按键都只唤醒待机页并吞掉本次完整手势；待机页亮起后再次短按确认，才进入摇卦页。

校时时先长按下键，再扫描屏幕二维码加入开放热点 `CyberYAO-Time`。手机通常会
自动弹出古纸像素风校时页；若未弹出，在浏览器输入 `http://192.168.4.1/`。页面会
自动读取并提交手机时间，失败时可手动重试；设备显示成功提示后关闭热点。若两分钟
内未完成，热点自动关闭并显示超时提示。

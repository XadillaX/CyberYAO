...
- 30 秒无操作后进入待机：白昼显示日晷，夜间显示刻漏，并展示时间、日期、电量、农历、干支、节气与宜忌。
- 支持通过蓝牙校准系统时间；长按下键可进入校时模式，并显示旋转进度及成功或超时状态。
...
| 按键 | 行为 |
| --- | --- |
| 确认短按 | 从待机进入摇卦页；在摇卦页投掷下一爻；完成六爻后进入详细解读 |
| 上短按 | 在摇卦页返回待机；在详细解读中向上滚动 |
| 下短按 | 在详细解读中向下滚动；待机变暗时，上／下键均只恢复亮度 |
| 确认短按（解读页） | 返回结果总览 |

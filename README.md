...
- 系统时间无效时会自动开启两分钟校时热点；也可长按下键手动开启
  `CyberYAO-Time` 和本地 HTTP 页面，全程不接入互联网。
...
- 支持手机 Wi-Fi 校时：屏幕分三步提示打开手机 Wi-Fi、连接开放热点
  `CyberYAO-Time`、等待页面弹出后自动校时；DHCP 广播本机 DNS，wildcard DNS
  与 iOS、Android、Windows 的联网探测入口会自动唤起本地校时页。页面自动提交
  手机本地年月日时分秒，成功后提示并关闭热点。
...
屏幕完全熄灭时，任意按键都只唤醒待机页并吞掉本次完整手势；待机页亮起后再次短按确认，才进入摇卦页。

当前固件版本从未成功校时，或系统时间无效时，设备启动后会自动开启校时热点；也可
随时长按下键手动开启。按屏幕提示打开手机 Wi-Fi 并连接开放热点
`CyberYAO-Time`，手机通常会自动弹出古纸像素风校时页；若未弹出，在浏览器输入
`http://192.168.4.1/`。页面会自动读取并提交手机时间，失败时可手动重试；设备显示
成功提示后关闭热点。成功校时标记与当前固件版本绑定，同一版本软件重启时不会重复
弹出。若两分钟内未完成，热点自动关闭并显示超时提示。

ESP32-C3 的系统时钟会在软件重启（包括长按上键进入 Recovery 前的重启）后继续
保留时间；设备没有后备 RTC，彻底断电后时间会丢失，下次启动会自动开启热点并需要
重新校时。

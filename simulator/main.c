...
static void play_coin_sound(SDL_AudioDeviceID device)
{
    ...
    if (SDL_QueueAudio(device, yaogui_coin_sound_pcm,
...
static void maintain_ambient_sound(SDL_AudioDeviceID device)
{
    ...
    const uint32_t loop_bytes =
        (uint32_t)(yaogui_ambient_sound_sample_count *
                   sizeof(yaogui_ambient_sound_pcm[0]));
    if (queued < loop_bytes / 2U) {
        if (SDL_QueueAudio(device, yaogui_ambient_sound_pcm,
                           loop_bytes) != 0) {
...
static int watch_sdl_event(void *userdata, SDL_Event *event)
{
    ...
    case SDLK_UP: s_keys |= KEY_UP; break;
    case SDLK_DOWN: s_keys |= KEY_DOWN; break;
    case SDLK_RETURN:
    case SDLK_KP_ENTER:
    case SDLK_SPACE: s_keys |= KEY_CONFIRM; break;
    case SDLK_l: s_keys |= KEY_CONFIRM_LONG; break;
    case SDLK_EQUALS:
    case SDLK_PLUS: s_keys |= KEY_SCALE_UP; break;
    case SDLK_MINUS: s_keys |= KEY_SCALE_DOWN; break;
    case SDLK_ESCAPE: s_keys |= KEY_RESET; break;
    case SDLK_q: s_keys |= KEY_QUIT; break;
    default: break;
    }
    return 1;
}
...
int main(int argc, char **argv)
{
    ...
    if (snapshot_path && snapshot_mode &&
        strcmp(snapshot_mode, "ready") == 0) {
        snapshot_now_ms = 0;
        ...
        for (size_t line = 0; line < YAOGUI_LINE_COUNT; line++) {
            ...
            yaogui_model_complete(&model, preview[line],
    ...
    while (running) {
        ...
        if (keys & KEY_UP) {
            if (!model.reading.open ||
                !yaogui_view_reading_scroll(view, -1)) {
                (void)yaogui_model_key(
                    &model, YAOGUI_KEY_UP, now_ms, now_ms);
            }
        }
        if (keys & KEY_DOWN) {
            if (!model.reading.open ||
                !yaogui_view_reading_scroll(view, 1)) {
                (void)yaogui_model_key(
                    &model, YAOGUI_KEY_DOWN, now_ms, now_ms);
            }
        }
        if ((keys & KEY_CONFIRM) &&
            yaogui_model_key(&model, YAOGUI_KEY_OK, now_ms,
                             now_ms ^ mock.sequence * 2654435761U)) {
            mock_backend_request(&mock, now_ms);
            play_coin_sound(audio_device);
        }
        if ((keys & KEY_CONFIRM_LONG) &&
            yaogui_model_key(&model, YAOGUI_KEY_OK_LONG, now_ms,
                             now_ms ^ mock.sequence * 2654435761U)) {
            mock_backend_request(&mock, now_ms);
            play_coin_sound(audio_device);
        }

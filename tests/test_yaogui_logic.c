...
static void test_minimum_roll_and_success(void) {
  ...
  yaogui_model_tick(&model,
                    1000 + YAOGUI_MIN_ROLL_MS + YAOGUI_LANDING_TOTAL_MS - 1);
  assert(model.phase == YAOGUI_ROLLING);
  yaogui_model_tick(&model,
                    1000 + YAOGUI_MIN_ROLL_MS + YAOGUI_LANDING_TOTAL_MS);
...
static void test_failure_and_retry(void) {
  ...
  assert(model.phase == YAOGUI_ROLLING);
  yaogui_model_tick(&model, YAOGUI_MIN_ROLL_MS + YAOGUI_LANDING_TOTAL_MS);
...
static void test_shell_to_trigram_mapping(void) {
  static const struct {
    ...
    const char* text;
      ...
      {{0, 0, 0}, YAOGUI_TRIGRAM_KUN, "☷", "坤", "元亨，利牝马之贞。"},
      {{1, 0, 0}, YAOGUI_TRIGRAM_ZHEN, "☳", "震", "亨。震来虩虩，笑言哑哑。"},
      {{0, 1, 0},
       YAOGUI_TRIGRAM_KAN,
       "☵",
       "坎",
      ...
      {{0, 0, 1},
       ...
       "艮",
       "艮其背，不获其身；行其庭，不见其人。"},
      {{1, 0, 1}, YAOGUI_TRIGRAM_LI, "☲", "离", "利贞，亨；畜牝牛，吉。"},
      {{0, 1, 1}, YAOGUI_TRIGRAM_XUN, "☴", "巽", "小亨，利有攸往，利见大人。"},
      {{1, 1, 1}, YAOGUI_TRIGRAM_QIAN, "☰", "乾", "元亨，利贞。"},
  };
...
static void test_six_line_flow_and_change(void) {
  ...
  for (size_t i = 0; i < YAOGUI_LINE_COUNT; i++) {
    ...
    yaogui_model_complete(
        &model, coins[i % 4U], YAOGUI_SOURCE_ESP32_RF, now + 1U);
...
static void test_all_hexagram_mappings(void) {
  ...
  static const char* const name_by_bits[64] = {
      "坤",   "复",   "师",   "临",   "谦", "明夷", "升", "泰",   "豫", "震",
      "解",   "归妹", "小过", "丰",   "恒", "大壮", "比", "屯",   "坎", "节",
      "蹇",   "既济", "井",   "需",   "萃", "随",   "困", "兑",   "咸", "革",
      "大过", "夬",   "剥",   "颐",   "蒙", "损",   "艮", "贲",   "蛊", "大畜",
      "晋",   "噬嗑", "未济", "睽",   "旅", "离",   "鼎", "大有", "观", "益",
      "涣",   "中孚", "渐",   "家人", "巽", "小畜", "否", "无妄", "讼", "履",
      "遁",   "同人", "姤",   "乾",
  };
  for (uint8_t bits = 0; bits < 64; bits++) {
    ...
    for (size_t line = 0; line < YAOGUI_LINE_COUNT; line++) {
      lines[line] =
          (bits & (1U << line)) ? YAOGUI_YOUNG_YANG : YAOGUI_YOUNG_YIN;
    }
    ...
    for (uint8_t moving = 0; moving < 64; moving++) {
      for (size_t line = 0; line < YAOGUI_LINE_COUNT; line++) {
        ...
        lines[line] = yang ? (changes ? YAOGUI_OLD_YANG : YAOGUI_YOUNG_YANG)
      ...
      assert(changed->number == king_wen_number_by_bits[bits ^ moving]);
    }
  }
...
static void test_text_data_completeness(void) {
  ...
  assert(strcmp(YAOGUI_HEXAGRAMS[0].special, "用九：见群龙无首，吉。") == 0);
...
static void test_reading_pagination_and_zhu_xi_primary(void) {
  for (uint8_t moving = 0; moving <= YAOGUI_LINE_COUNT; moving++) {
    ...
    for (uint8_t line = 0; line < YAOGUI_LINE_COUNT; line++) {
      lines[line] = line < moving ? YAOGUI_OLD_YANG : YAOGUI_YOUNG_YANG;
    }
    ...
    for (uint8_t line = 0; line < YAOGUI_LINE_COUNT; line++) {
      assert(strstr(plan.pages[0].text, plan.pages[0].hexagram->lines[line]) !=
             NULL);
    }
    assert(strstr(plan.pages[0].text, plan.pages[0].hexagram->text) != NULL);
    assert(strstr(plan.pages[0].text, plan.pages[0].hexagram->special) != NULL);
    if (moving > 0) {
      assert(plan.pages[1].type == YAOGUI_READING_CHANGED_GUACI);
      for (uint8_t line = 0; line < YAOGUI_LINE_COUNT; line++) {
    ...
    if (moving == 4 || moving == 5) {
      assert(plan.pages[primary_index].type == YAOGUI_READING_CHANGED_GUACI);
      assert(primary_index == 1U);
    }
    if (moving == 6) {
      assert(plan.pages[primary_index].type == YAOGUI_READING_PRIMARY_GUACI);
...
static void test_reading_key_flow(void) {
  ...
  for (size_t line = 0; line < YAOGUI_LINE_COUNT; line++) {
    model.lines[line] = line == 0 ? YAOGUI_OLD_YANG : YAOGUI_YOUNG_YANG;
  }
